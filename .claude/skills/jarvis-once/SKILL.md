...
### ① 로컬 (편집 중인 코드만으로 판단)
- 버그 잠재성: force unwrap, nil 처리 누락, 경계 케이스 — **HIGH**
- 메모리 누수: 클로저·델리게이트 강한 참조 순환(`[weak self]` 누락) — **HIGH**

### ② 프로젝트 전역 (코드베이스를 알아야 판단 — Read/Grep/Glob로 탐색)
- 파급 영향: 공유 ViewModel·프로토콜·공용 모델 변경이 다른 화면에 미치는 영향 — **HIGH**
- 중복/재발명: 같은 로직이 이미 있는데 또 만들려는 시도 — **MEDIUM**
- 아키텍처 일탈: 팀 컨벤션(MVVM, 공용 네트워킹 레이어 등)에서 벗어남 — **MEDIUM**

### ③ 의도 기반 (Jira·Confluence·Figma MCP가 있어야 판단)
- 스펙/디자인 불일치: Confluence 스펙·Figma 화면과 코드가 어긋남 — **HIGH**
- 누락된 상태 처리: 디자인의 empty/error 화면이 코드에 없음 — **MEDIUM**
- 다음 스텝 누락: 레이아웃은 됐는데 데이터 소스 연결 등 후속 작업이 빠짐 — **LOW~MEDIUM**
